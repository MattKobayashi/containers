...
def is_within_one_hour(target_datetime):
  ...
  Returns:
    True if the target datetime is within one hour of the current time, False otherwise.
  """
  now = datetime.now()
...
if not environ['PUSHOVER_APP_TOKEN'] or not environ['PUSHOVER_USER_KEY']:
  print("ERROR: Please set the PUSHOVER_APP_TOKEN and PUSHOVER_USER_KEY environment variables.")
  exit()

blog_feed = feedparser.parse("https://blog.vyos.io/rss.xml")
latest_post = blog_feed.entries[0]

print(f"Latest blog entry: {latest_post.title}")
print(f"Published: {latest_post.published}")

if is_within_one_hour(datetime.fromtimestamp(mktime(latest_post.published_parsed))):
  conn = http.client.HTTPSConnection("api.pushover.net:443")
  conn.request("POST", "/1/messages.json",
    urllib.parse.urlencode({
      "token": f"{environ['PUSHOVER_APP_TOKEN']}",
      "user": f"{environ['PUSHOVER_USER_KEY']}",
      "message": f"There's a new post on the VyOS blog. Link: {latest_post.link}",
