...
if [ -f /run/secrets/DEVICE_PASS ]; then
	DEVICE_PASS=$(cat /run/secrets/DEVICE_PASS)
fi
...
if [ -f /run/secrets/NOTIFY_TELEGRAM_TOKEN ]; then
	NOTIFY_TELEGRAM_TOKEN=$(cat /run/secrets/NOTIFY_TELEGRAM_TOKEN)
fi

if [ -f /run/secrets/NOTIFY_SLACK_TOKEN ]; then
	NOTIFY_SLACK_TOKEN=$(cat /run/secrets/NOTIFY_SLACK_TOKEN)
fi
